...
export class DerivWebSocket extends EventEmitter {
  ...
  private activeZones: SupplyDemandZone[] = [];
  ...
  private analyzeCandles(candles: any[], symbol: string) {
    if (!candles || candles.length < 50) return;

    // Convert to standard format
    const standardCandles = candles.map((c: any) => ({
  ...
  private detectZones(candles: DerivCandle[], symbol: string, timeframe: number) {
  ...
  private isConsolidation(bars: DerivCandle[], threshold: number): boolean {
  ...
  getCandles(symbol: string, timeframe: number = 60, count: number = 100) {
    this.send({
  ...
  updateSettings(settings: {
    ...
    if (settings.minSignalGap) this.minSignalGap = settings.minSignalGap;
